...
export default function OrdersPage() {
  ...
  async function saveEdit() {
    const { error } = await supabaseBrowser.from("orders").update({
      name: editOrder.name, email: editOrder.email, domain: editOrder.domain,
      plan: editOrder.plan, total: parseFloat(editOrder.total),
      notes: editOrder.notes, status: editOrder.status,
      setup_date: editOrder.setup_date || '', setup_time: editOrder.setup_time || '',
  ...
  function setDesign(index, value) { const u = [...(editOrder.selected_designs||[])]; u[index]=value; setEditOrder({...editOrder,selected_designs:u}) }
  function addDesign() { const c=editOrder.selected_designs||[]; if(c.length>=3)return; setEditOrder({...editOrder,selected_designs:[...c,'']}) }
  function removeDesign(index) { const u=[...(editOrder.selected_designs||[])]; u.splice(index,1); setEditOrder({...editOrder,selected_designs:u}) }
  function setDomain(index, value) { const u=[...(editOrder.addon_domain_list||[])]; u[index]=value; setEditOrder({...editOrder,addon_domain_list:u}) }
  function addDomain() { setEditOrder({...editOrder,addon_domain_list:[...(editOrder.addon_domain_list||[]),'']}) }
  function removeDomain(index) { const u=[...(editOrder.addon_domain_list||[])]; u.splice(index,1); setEditOrder({...editOrder,addon_domain_list:u}) }
  function getAvailableDesigns(ci) { const chosen=(editOrder.selected_designs||[]).filter((d,i)=>i!==ci&&d); return DESIGNS.filter(d=>!chosen.includes(d)) }
            ...
            <div style={{ background:'white', borderRadius:20, padding:32, width:'100%', maxWidth:620, maxHeight:'90vh', overflowY:'auto' }}>
              ...
              <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:14, marginBottom:14 }}>
                ...
                <div><label style={labelStyle}>Plan</label>
                  <select value={editOrder.plan||''} onChange={e=>setEditOrder({...editOrder,plan:e.target.value})} style={selectStyle}>
                    <option value="Starter">Starter</option><option value="Growth">Growth</option><option value="Studio">Studio</option>
                  </select>
                </div>
                ...
                <div><label style={labelStyle}>Status</label>
                  <select value={editOrder.status||'pending'} onChange={e=>setEditOrder({...editOrder,status:e.target.value})} style={selectStyle}>
                    <option value="pending">Pending</option><option value="active">Active</option><option value="cancelled">Cancelled</option>
                  </select>
                </div>
              ...
              <div style={{ marginBottom:16, padding:16, background:'#f8f7f4', borderRadius:12, border:'1px solid #e2e8f0' }}>
                <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:10 }}>
                  ...
                  {(editOrder.selected_designs||[]).length < 3 && <button onClick={addDesign} style={{ padding:'3px 10px', background:'#0F172A', color:'white', border:'none', borderRadius:6, fontSize:11, cursor:'pointer' }}>+ Add</button>}
                </div>
                {(editOrder.selected_designs||[]).map((d,i) => (
                  <div key={i} style={{ display:'flex', gap:8, marginBottom:8, alignItems:'center' }}>
                    ...
                    {(editOrder.selected_designs||[]).length > 1 && <button onClick={()=>removeDesign(i)} style={{ padding:'6px 10px', background:'#fff5f5', color:'#dc2626', border:'1px solid #fee2e2', borderRadius:6, fontSize:12, cursor:'pointer' }}>✕</button>}
                  </div>
                ))}
              </div>
              <div style={{ marginBottom:16, padding:16, background:'#f8f7f4', borderRadius:12, border:'1px solid #e2e8f0' }}>
                ...
                {(editOrder.addon_domain_list||[]).map((d,i) => (
                  <div key={i} style={{ display:'flex', gap:8, marginBottom:8, alignItems:'center' }}>
                    ...
                    {(editOrder.addon_domain_list||[]).length > 1 && <button onClick={()=>removeDomain(i)} style={{ padding:'6px 10px', background:'#fff5f5', color:'#dc2626', border:'1px solid #fee2e2', borderRadius:6, fontSize:12, cursor:'pointer' }}>✕</button>}
                  </div>
                ))}
              </div>
