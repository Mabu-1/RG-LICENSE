...
export default function OrdersPage() {
  ...
  async function deleteOrder(id) {
    ...
    if (error) { console.error(error); setMsg("Delete failed!"); return; }
  ...
  async function saveEdit() {
    const { error } = await supabaseBrowser.from("orders").update({
      ...
      selected_designs: editOrder.selected_designs || [],
    }).eq("id", editOrder.id);
    if (error) { console.error(error); setMsg("Save failed!"); return; }
      ...
      <div style={{ maxWidth: 1300, margin: "0 auto" }}>
        ...
        {editOrder && (
          <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.5)', zIndex:1000, display:'flex', alignItems:'center', justifyContent:'center', padding:24 }}>
            <div style={{ background:'white', borderRadius:20, padding:32, width:'100%', maxWidth:600, maxHeight:'90vh', overflowY:'auto' }}>
              ...
              <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:14, marginBottom:14 }}>
                ...
                <div>
                  ...
                  <select value={editOrder.plan || ''} onChange={e=>setEditOrder({...editOrder,plan:e.target.value})} style={inputStyle}>
                ...
                <div>
                  ...
                  <select value={editOrder.status || 'pending'} onChange={e=>setEditOrder({...editOrder,status:e.target.value})} style={inputStyle}>
              ...
              <div style={{ marginBottom:14 }}>
                <label style={labelStyle}>Extra Domain</label>
                <input
                  value={Array.isArray(editOrder.addon_domain_list) ? editOrder.addon_domain_list.join(', ') : ''}
                  onChange={e=>setEditOrder({...editOrder, addon_domain_list: e.target.value.split(',').map(d=>d.trim()).filter(Boolean)})}
                  placeholder="domain1.com, domain2.com"
                  style={inputStyle}
                />
              </div>
              <div style={{ marginBottom:14 }}>
                <label style={labelStyle}>Selected Designs</label>
                <input
                  value={Array.isArray(editOrder.selected_designs) ? editOrder.selected_designs.join(', ') : ''}
                  onChange={e=>setEditOrder({...editOrder, selected_designs: e.target.value.split(',').map(d=>d.trim()).filter(Boolean)})}
                  placeholder="Natural, Equal Height, Masonry"
                  style={inputStyle}
                />
              </div>
              ...
              <table style={{ width: "100%", borderCollapse: "collapse" }}>
                <thead>
                  <tr style={{ background: "#f8f7f4", borderBottom: "2px solid #e2e8f0" }}>
                    {["Date","Name","Email","Domain","Plan","Designs","Extra Domain","Setup","Total","Status",""].map(h => (
                      ...
                      <tr key={o.id} style={{ borderBottom: "1px solid #f1f5f9", background: i % 2 === 0 ? "white" : "#fafafa" }}>
                        ...
                        <td style={{ padding: "14px 16px", fontSize: 12 }}>
                          {designs.length > 0 ? designs.map((d,idx) => (
                            <span key={idx} style={{ display:"inline-block", padding:"2px 8px", background: idx===0 ? "#f1f5f9" : "#FEF3C7", borderRadius:20, fontSize:11, marginRight:4, color: idx===0 ? "#0F172A" : "#92400E", fontWeight:600 }}>{d}</span>
