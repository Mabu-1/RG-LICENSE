...
export default function MediaPage() {
  ...
  async function uploadFile(e) {
    ...
    for (const file of files) {
      ...
      setUploads(prev => [{ url: urlData.publicUrl, name: file.name, type: file.type }, ...prev])
    }
    ...
    <div style={{ minHeight:'100vh', background:'#f8f7f4', padding:'40px 24px' }}>
      <div style={{ maxWidth:900, margin:'0 auto' }}>
        ...
        <div style={{ background:'white', borderRadius:16, padding:32, marginBottom:24, border:'2px dashed #e2e8f0', textAlign:'center' }}>
          ...
          <div style={{ fontSize:13, color:'#64748b', marginBottom:20 }}>Supported: JPG, PNG, GIF, WEBP, MP4, MOV — multiple files at once</div>
        ...
        {uploads.length > 0 && (
          <div style={{ background:'white', borderRadius:16, padding:28, border:'1px solid #e2e8f0' }}>
            <h2 style={{ fontSize:16, fontWeight:700, color:'#0F172A', marginBottom:20 }}>Uploaded This Session ({uploads.length})</h2>
            <div style={{ display:'flex', flexDirection:'column', gap:12 }}>
              {uploads.map((u, i) => (
                <div key={i} style={{ display:'flex', alignItems:'center', gap:16, padding:12, background:'#f8f7f4', borderRadius:10, border:'1px solid #e2e8f0' }}>
                  <div style={{ width:64, height:64, borderRadius:8, overflow:'hidden', flexShrink:0, background:'#e2e8f0', display:'flex', alignItems:'center', justifyContent:'center' }}>
                  ...
                  <div style={{ flex:1, minWidth:0 }}>
                    <div style={{ fontSize:12, fontWeight:600, color:'#0F172A', marginBottom:4, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{u.name}</div>
                  ...
                  <button onClick={() => copyUrl(u.url)}
                    style={{ padding:'8px 16px', background:'#0F172A', color:'white', border:'none', borderRadius:6, fontSize:12, fontWeight:600, cursor:'pointer', whiteSpace:'nowrap', flexShrink:0 }}>
                    Copy URL
                  </button>
                </div>
              ))}
