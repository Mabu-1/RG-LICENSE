    ...
    async function prepare() {
      try {
        const res = await fetch("/api/stripe/create-checkout", {
          ...
          body: JSON.stringify({ name: name || "Customer", email: email || "customer@example.com", orderId, total: parseFloat(total) }),
        });
  ...
  async function handlePay() {
    ...
    if (paymentIntent.status === "succeeded") {
      await fetch("/api/stripe/confirm-order", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ orderId, customerId, paymentMethodId: paymentIntent.payment_method, paymentIntentId: paymentIntent.id }),
      });
  ...
  return (
    <div>
      <div style={{ border: "1.5px solid #BFDBFE", borderRadius: 12, padding: "16px 18px", background: "#F8FAFF", marginBottom: 16 }}>
        <div style={{ fontSize: 11, fontWeight: 700, color: "#64748b", textTransform: "uppercase", letterSpacing: 1, marginBottom: 12 }}>Card Details</div>
        <CardElement options={CARD_STYLE} />
      </div>
      {error && (
        <div style={{ color: "#ef4444", fontSize: 12, marginBottom: 12, padding: "8px 12px", background: "#FEF2F2", borderRadius: 8, border: "1px solid #FCA5A5" }}>{error}</div>
      )}
      <button
        onClick={handlePay}
        disabled={loading || !stripe || !clientSecret}
        style={{
          width: "100%", padding: "15px", background: loading ? "#94a3b8" : "#2563EB",
          color: "white", border: "none", borderRadius: 100, fontSize: 15, fontWeight: 700,
          cursor: loading ? "not-allowed" : "pointer", transition: "all 0.2s",
          boxShadow: loading ? "none" : "0 8px 28px rgba(37,99,235,0.3)", marginBottom: 12,
        }}
      >
  ...
  return (
    <>
      <style>{`
        ...
        .bk-box { background: white; border-radius: 24px; padding: 40px 36px; border: 1px solid #BFDBFE; box-shadow: 0 8px 40px rgba(37,99,235,0.08); max-width: 440px; width: 100%; }
        ...
        .bk-form-title { font-size: 13px; font-weight: 700; color: #0F172A; margin-bottom: 16px; text-align: center; }
