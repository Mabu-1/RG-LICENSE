...
function CheckoutContent() {
  ...
  function validate() {
    ...
    domains.forEach((d, i) => { if (i > 0 && !d.trim()) e[`domain${i}`] = "Required"; });
    setErrors(e);
  ...
  async function handleSubmit() {
    ...
    if (error) { console.error("Order save error:", error); setLoading(false); return; }
    router.push(`/checkout/book?name=${encodeURIComponent(form.name)}&total=${fullTotal}`);
  }

  const inp = { width: "100%", padding: "11px 14px", border: "1px solid #BFDBFE", borderRadius: 10, fontSize: 14, boxSizing: "border-box", fontFamily: "inherit", outline: "none", background: "white" };
  ...
  return (
    <>
      <style>{`
        ...
        .co-card-title { font-size: 15px; font-weight: 700; color: #0F172A; margin-bottom: 6px; }
        .co-card-sub { font-size: 13px; color: #64748b; margin-bottom: 20px; }
        ...
        /* Summary */
        .co-summary { background: white; border-radius: 16px; padding: 24px; border: 1px solid #BFDBFE; position: sticky; top: 24px; }
        .co-trial-box { background: #F0FDF4; border: 1.5px solid #86EFAC; border-radius: 14px; padding: 24px 20px; text-align: center; margin-bottom: 20px; }
        .co-trial-badge { display: inline-flex; align-items: center; gap: 6px; background: white; border: 1px solid #86EFAC; border-radius: 100px; padding: 3px 12px; margin-bottom: 10px; }
        ...
        .co-trial-price { display: flex; align-items: baseline; justify-content: center; gap: 2px; margin-bottom: 6px; }
        .co-trial-dollar { font-family: serif; font-size: 24px; font-weight: 900; color: #22C55E; }
        .co-trial-num { font-family: serif; font-size: 72px; font-weight: 900; color: #22C55E; letter-spacing: -4px; line-height: 1; }
        .co-trial-then { font-size: 12px; color: #64748b; }
        .co-trial-then strong { color: #0F172A; }
        .co-srow { display: flex; justify-content: space-between; font-size: 13px; padding: 7px 0; border-bottom: 1px solid #F1F5F9; }
        .co-srow:last-of-type { border: none; }
        .co-stotal { display: flex; justify-content: space-between; align-items: center; padding-top: 12px; border-top: 2px solid #EFF6FF; margin-top: 6px; }
        .co-submit { width: 100%; padding: 16px; background: #2563EB; color: white; border: none; border-radius: 100px; font-size: 15px; font-weight: 700; cursor: pointer; transition: all 0.2s; box-shadow: 0 8px 24px rgba(37,99,235,0.3); margin-top: 12px; margin-bottom: 10px; }
        ...
        .co-trust { display: flex; justify-content: center; gap: 14px; margin-top: 10px; flex-wrap: wrap; }
          ...
          <div className="co-grid">
            ...
            <div>
              ...
              <div className="co-card">
                ...
                <div className="co-field">
                  ...
                  <input value={form.whatsapp} onChange={e => setForm({ ...form, whatsapp: e.target.value })} placeholder="+1 234 567 8900" style={{ ...inp, borderColor: errors.whatsapp ? "#ef4444" : "#BFDBFE" }} />
              ...
              <div className="co-card">
                ...
                <div className="co-card-sub">First 3 domains are included free. Each additional domain is +$10.</div>
                  ...
                  <div key={i} className="co-domain-row">
                    <div style={{ flex: 1 }}>
                      <label style={{ ...lbl, display: "flex", alignItems: "center", gap: 6 }}>
                        ...
                        {i >= 1 && i <= 2 && <span style={{ color: "#22C55E", fontSize: 11, fontWeight: 700, background: "#F0FDF4", padding: "1px 8px", borderRadius: 100, border: "1px solid #86EFAC" }}>Free</span>}
                        {i >= 3 && <span style={{ color: "#2563EB", fontSize: 11, fontWeight: 700, background: "#EFF6FF", padding: "1px 8px", borderRadius: 100, border: "1px solid #BFDBFE" }}>+$10</span>}
                      </label>
                      <input value={d} onChange={e => updateDomain(i, e.target.value)} placeholder={`store${i > 0 ? i + 1 : ""}.myshopify.com`} style={{ ...inp, borderColor: errors[`domain${i}`] ? "#ef4444" : "#BFDBFE" }} />
                    ...
                    {i > 0 && (
                      <button className="co-remove-btn" onClick={() => removeDomain(i)}>×</button>
                    )}
                  </div>
                ))}
                ...
                <button className="co-add-btn" onClick={addDomain}>
                  ...
                  Add another domain {domains.length >= 3 ? "(+$10 each)" : "(free)"}
                </button>
              </div>
            </div>
            ...
            <div>
              <div className="co-summary">
                <div style={{ fontSize: 12, fontWeight: 700, color: "#64748b", textTransform: "uppercase", letterSpacing: 1, marginBottom: 16 }}>Order Summary</div>
                ...
                <div>
                  <div className="co-srow">
                    <span style={{ color: "#64748b" }}>{plan.name} Plan</span>
                    <span style={{ fontWeight: 700 }}>${plan.price}{plan.cents}</span>
                  </div>
                  <div className="co-srow">
                    <span style={{ color: "#64748b" }}>Domains (1–3)</span>
                    <span style={{ fontWeight: 700, color: "#22C55E" }}>Free</span>
                  </div>
                  {domains.length > 3 && (
                    <div className="co-srow">
                      <span style={{ color: "#64748b" }}>Extra domains ({domains.length - 3})</span>
                      <span style={{ fontWeight: 700, color: "#2563EB" }}>+${addonCost}</span>
                    </div>
                  )}
                </div>

                <div className="co-stotal">
                  <div>
                  ...
                  <div style={{ fontFamily: "serif", fontSize: 26, fontWeight: 900, color: "#0F172A", letterSpacing: -1 }}>${fullTotal.toFixed(2)}</div>
                </div>

                <button onClick={handleSubmit} disabled={loading} className="co-submit">
                  {loading ? "⏳ Saving..." : "Continue to Book a Call →"}
                </button>
