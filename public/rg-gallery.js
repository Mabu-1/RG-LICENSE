(async () => {
  ...
  try {
    const res = await fetch('https://YOUR-VERCEL-URL/api/check?domain=' + encodeURIComponent(domain));
  ...
  function initGallery(settings) {
    ...
    const COL_PRODUCT=0,COL_RATING=1,COL_AUTHOR=2,COL_EMAIL=3,COL_BODY=4,COL_DATE=5,COL_PHOTO=6,COL_VERIFIED=7,COL_VARIANT=8;
    ...
    function parseAndRender(csvText){
      ...
      rows.forEach((row,idx)=>{
        ...
        allReviews.push({
          id:idx,productHandle:row[COL_PRODUCT]||"",rating:parseInt(row[COL_RATING])||5,
